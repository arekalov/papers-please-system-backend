-- Seed data for Papers Please System (Production version - Basic Schema)
-- Работает с базовой схемой БД БЕЗ миграций V2, V6, V7, V8
-- Для полной версии с новыми фичами используйте seed-data-prod.sql
--
...
-- Data for table: documents (без attach_to_profile)
INSERT INTO documents (id, uploaded_at, document_type, body, issued_at, expires_at, owner_id) 
VALUES ('a05e96e7-0deb-4147-a1be-2a06f07e5cfb', '2025-11-23 19:10:33.979387+00:00', 'PASSPORT', '{"number": "123456789", "series": "AB", "fullName": "Лейла Мигрантова"}', '2020-01-01 00:00:00+00:00', '2030-01-01 00:00:00+00:00', '091b55a5-f94b-429d-8569-9dbfd050ae3c') ON CONFLICT (id) DO NOTHING;

INSERT INTO documents (id, uploaded_at, document_type, body, issued_at, expires_at, owner_id) 
VALUES ('b16fa8f8-1efc-5258-b2cf-3b17f18f6d0a', '2025-11-23 19:15:00.000000+00:00', 'WORK_PERMIT', '{"number": "WP-987654", "employer": "ООО Строймонтаж"}', '2024-01-01 00:00:00+00:00', '2025-12-31 23:59:59+00:00', '091b55a5-f94b-429d-8569-9dbfd050ae3c') ON CONFLICT (id) DO NOTHING;

-- Data for table: ticket_relations (связанные тикеты)
INSERT INTO ticket_relations (ticket_id, related_ticket_id) 
VALUES ('5ed93a7e-cb4c-40c3-b4f3-be4ad5e2588b', 'a4f2b630-3067-4279-8f0e-24f5664e0602') ON CONFLICT DO NOTHING;

INSERT INTO ticket_relations (ticket_id, related_ticket_id) 
VALUES ('a4f2b630-3067-4279-8f0e-24f5664e0602', '5ed93a7e-cb4c-40c3-b4f3-be4ad5e2588b') ON CONFLICT DO NOTHING;

INSERT INTO ticket_relations (ticket_id, related_ticket_id) 
VALUES ('33333333-3333-3333-3333-333333333333', '44444444-4444-4444-4444-444444444444') ON CONFLICT DO NOTHING;

INSERT INTO ticket_relations (ticket_id, related_ticket_id) 
VALUES ('44444444-4444-4444-4444-444444444444', '33333333-3333-3333-3333-333333333333') ON CONFLICT DO NOTHING;

-- Data for table: events (со shift_id, БЕЗ priority и upk_id)
...
SELECT 'Ticket Relations: ' || COUNT(*) FROM ticket_relations;
